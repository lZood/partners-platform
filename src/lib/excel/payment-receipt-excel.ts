...
const USD_FMT = '"$"#,##0.00';
const MXN_FMT = '"$"#,##0.00" MXN"';

const INK = "FF111827";
const WHITE = "FFFFFFFF";
const ZEBRA = "FFF9FAFB";
const ACCENT = "FFDC2626";
const MUTED = "FF6B7280";

/**
 * Build a single-payment receipt as an .xlsx workbook, mirroring the redesigned
 * PDF: header, datos, per-product distribution table, extra concepts and totals.
 */
export async function generatePaymentReceiptExcel(
  ...
  // ---- Header: PARTNER | PAGO ----
  const headerRow = ws.addRow([data.partnerName.toUpperCase(), "", "", "PAGO"]);
  headerRow.height = 30;
  headerRow.getCell(1).font = { bold: true, size: 18, color: { argb: INK } };
  headerRow.getCell(1).alignment = { vertical: "middle" };
  ws.mergeCells(headerRow.number, 1, headerRow.number, 3);
  headerRow.getCell(4).font = { bold: true, size: 18, color: { argb: INK } };
  ...
  for (const [label, value] of datos) {
    ...
    r.getCell(2).font = { color: { argb: MUTED } };
  ...
  if (data.products.length > 0) {
    ...
    for (const row of data.products) {
      ...
      if (
        ...
        sep.getCell(1).font = { bold: true, italic: true, size: 9, color: { argb: MUTED } };
  ...
  if (data.concepts.length > 0) {
    ...
    ch.getCell(1).font = { bold: true, color: { argb: INK }, size: 10 };
    ch.getCell(3).font = { bold: true, color: { argb: INK }, size: 10 };
    ch.getCell(3).alignment = { horizontal: "right" };
    ch.getCell(4).font = { bold: true, color: { argb: INK }, size: 10 };
    ...
    for (const c of data.concepts) {
      ...
      r.getCell(4).numFmt = MXN_FMT;
      if (c.isDeduction) {
        r.getCell(3).font = { color: { argb: ACCENT } };
        r.getCell(4).font = { color: { argb: ACCENT } };
      }
    }
    ws.addRow([]);
  }
...
function addTotalRow(
  ...
  labelCell.font = { bold: opts.bold ?? false, color: { argb: opts.color ?? INK } };
  valueCell.numFmt = fmt;
  valueCell.alignment = { horizontal: "right" };
  valueCell.font = { bold: opts.bold ?? false, color: { argb: opts.color ?? INK } };
  if (opts.fill) {
    labelCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: opts.fill } };
    valueCell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: opts.fill } };
  }
}
